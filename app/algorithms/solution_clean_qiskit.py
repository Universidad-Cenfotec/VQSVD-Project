...
class VQSVD:
    def __init__(self,
        ...
        self.U = self.get_unitary_matrix(self.theta)
        self.V = self.get_unitary_matrix(self.phi)
    ...
    def get_unitary_matrix(self, params: np.ndarray) -> np.ndarray:
        ...
        for idx in range(dim):
            ...
            for q, bit in enumerate(bstr):
                if bit == '1':
                    qc.x(q)
            qc = qc.compose(self.ansatz(params))
            qc.save_statevector()
    ...
    def get_matrix_U(self) -> np.ndarray:
        return self.U

    def get_matrix_V(self) -> np.ndarray:
        return self.V

    def loss_func(self, flat_vars: np.ndarray):
        ...
        U = self.get_unitary_matrix(theta)
        V = self.get_unitary_matrix(phi)
    ...
    def train(self, beta1=0.9, beta2=0.999, epsilon=1e-8, n_jobs=8):
        chunk = self.depth * 2 * self.num_qubits
        ...
        for t in range(1, self.iterations + 1):
            ...
            loss, svals = self.loss_func(flat_vars)
...
class LossPlot:
    ...
    def loss_plot(self, losses: list):
        """
    ...
    def plot_singular_values_comparison(self, rank: int, vqsvd: list, svd: list):
        """
    ...
    def plot_matrix_as_image(self, matrix: np.ndarray, title: str = "Reconstructed Matrix"):
        """
...
loss_history, svals_history = model.train(n_jobs=10)

U_final = model.get_matrix_U()
Vh_final = model.get_matrix_V().conj().T

# -------------------------------
# 4) Plot loss curve
# -------------------------------
plotter = LossPlot(save_path="../results/")
plotter.loss_plot(loss_history)
...
err_local, err_subfull, err_svd = model.frobenius_norm_error(
...
plotter.plot_singular_values_comparison(rank=rank, vqsvd=svals_q, svd=svals_exact)
...
plotter.plot_matrix_as_image(reconstructed, title=f"VQSVD Reconstruction (rank={rank})")