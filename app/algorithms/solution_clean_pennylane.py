...
class VQSVD():
    def __init__(self, matrix: pnp.ndarray, 
        ...
        # Parametros iniciales para U y V
        ...
        self.basis_states = [''.join(bits) for bits in product('01', repeat=num_qubits)]

        @qml.qnode(self.dev)
        def _basis_circuit(state_bin, params):
            # Aplicar PauliX a los bits en 1 para preparar el estado base
            for j in [i for i, bit in enumerate(reversed(state_bin)) if bit == "1"]:
                qml.PauliX(wires=j)
            self.ansatz(params, wires=range(self.num_qubits))
            return qml.state()
        self.basis_circuit = _basis_circuit

        # Inicializar matrices unitarias
    ...
    def ansatz(self, params, wires):
        ...
        for d in range(depth):
            ...
            for q, w in enumerate(wires):
                qml.RY(ry_layer[q], wires=w)
            for q in range(len(wires) - 1):
                qml.CNOT(wires=[wires[q], wires[q + 1]])

    def get_unitary_matrix(self, params):
        matrix_columns = []
        for b in self.basis_states:
            final_state = self.basis_circuit(b, params)
    ...
    def loss_func(self, theta, phi):
        ...
        M_eff = U_dag @ self.M @ V
        loss = pnp.array(0.0)
        singular_values = []

        for i in range(self.rank):
            val = pnp.real(M_eff[i, i])
            loss -= pnp.real(self.weight[i]) * val
    ...
    def train(self):
        ...
        # Actualizar las matrices finales
        self.U = self.get_unitary_matrix(self.theta)
        self.V = self.get_unitary_matrix(self.phi)
